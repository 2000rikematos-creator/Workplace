...
async function addNewTask(req:Request,res:Response,next:NextFunction){
    ...
    const taskExists = await pool.query("SELECT * from tasks where task = $1 AND workplace_id = $2",[customReq.body.task,customReq.workplace.id])
        if(taskExists.rows.length>0){throw new httpError("There's already a task with this name",422)}
...
const customReq = req as deleteTaskRequest
    ...
    if(taskIsActive.rows.length>0){throw new httpError("This task is active with one or more team members, please finish first",422)}    
    if(taskExists.rows.length<1){throw new httpError("Task not found", 404)}
